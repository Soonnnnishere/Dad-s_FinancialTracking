...
/**
 * Inner form holds the local edit state. The parent gives it a fresh `key`
 * for each open so React remounts and `useState` initializers run again —
 * cleaner than useEffect-based reset (React 19 idiom).
 */
function QuickAddForm({
  ...
  // Derived: if the user's pick still fits the current kind, use it; otherwise
  // fall back to the first matching category. No setState-in-effect needed.
  const effectiveCategoryId =
    pickedId && filtered.some((c) => c.id === pickedId)
      ? pickedId
      : filtered[0]?.id ?? null
